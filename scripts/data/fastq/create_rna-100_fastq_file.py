...
if __name__ == "__main__":
    ...
    # Step 3. Fetch TP53 sequence and create a somatic SNV at position 7674225 (C>A)
    ...
    for _,row in df_exons_tp53.iterrows():
        ...
        if exon_number == 7:
            # Create a SNV at position 7674225
            ...
            tumor_sequence = normal_sequence[:local_position] + 'A' + normal_sequence[local_position+1:]
        else:
    ...
    create_fastq_file(
        ...
        output_fastq_file='../../../test/data/fastq/rna-100-tumor_long-read.fastq',
        num_reads=[1,1],
        stranded=True
    )
    ...
    pd.DataFrame(data).to_csv('../../../test/data/tsv/ground_truth/rna-100-tumor_ground_truth.tsv', sep='\t', index=False)
    pd.DataFrame(data).to_csv('/Users/leework/Documents/Research/projects/project_exacto/exacto/exacto/exacto-caller/src/tests/data/tsv/ground_truth/rna-100-tumor_ground_truth.tsv', sep='\t', index=False)
