...
if __name__ == "__main__":
    ...
    # Step 3. Fetch TP53 sequence and create an alternative 5-prime splice site (chr17:7676099)
    ...
    for _,row in df_exons_tp53.iterrows():
        ...
        if exon_number == 4:
            # Create an alternative 5 prime splice site
    ...
    create_fastq_file(
        ...
        output_fastq_file='../../../test/data/fastq/rna-104-tumor_long-read.fastq',
        num_reads=[1,1],
        stranded=True
    )
    ...
    pd.DataFrame(data).to_csv('../../../test/data/tsv/ground_truth/rna-104-tumor_ground_truth.tsv', sep='\t', index=False)
    pd.DataFrame(data).to_csv('/Users/leework/Documents/Research/projects/project_exacto/exacto/exacto/exacto-caller/src/tests/data/tsv/ground_truth/rna-104-tumor_ground_truth.tsv', sep='\t', index=False)
