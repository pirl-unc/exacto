...
if __name__ == "__main__":
    ...
    # Step 3. Fetch WRAP53 sequence
    df_transcript_wrap53 = gencode.df_transcripts[gencode.df_transcripts['transcript_id_stable'] == 'ENST00000698746']
    df_exons_wrap53 = gencode.df_exons[gencode.df_exons['transcript_id'] == df_transcript_wrap53['transcript_id'].values[0]]
    df_exons_wrap53.sort_values(by=['number'], ascending=True, inplace=True) # WRAP53 is on the forward strand
    wrap53_tumor_sequence = ''
    wrap53_normal_sequence = ''
    for _,row in df_exons_wrap53.iterrows():
        ...
        if exon_number == 7:
            # Create a fusion (chr17:7701730-7727200)
            reference_position = 7701730
            local_position = len(normal_sequence) - (end - reference_position) - 1
            wrap53_tumor_sequence = wrap53_tumor_sequence + normal_sequence[:local_position+1]
        if exon_number < 7:
            wrap53_tumor_sequence = wrap53_tumor_sequence + normal_sequence
        wrap53_normal_sequence = wrap53_normal_sequence + normal_sequence

    # Step 4. Fetch DNAH2 sequence
    ...
    tumor_sequence = wrap53_tumor_sequence + dnah2_tumor_sequence
    create_fastq_file(
        sequences=[tumor_sequence,wrap53_normal_sequence,dnah2_normal_sequence],
        output_fastq_file='../../../test/data/fastq/rna-103-tumor_long-read.fastq',
        num_reads=[1,1,1],
        stranded=True
    )
    ...
    pd.DataFrame(data).to_csv('../../../test/data/tsv/ground_truth/rna-103-tumor_ground_truth.tsv', sep='\t', index=False)
    pd.DataFrame(data).to_csv('/Users/leework/Documents/Research/projects/project_exacto/exacto/exacto/exacto-caller/src/tests/data/tsv/ground_truth/rna-103-tumor_ground_truth.tsv', sep='\t', index=False)
