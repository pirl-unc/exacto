...
if __name__ == "__main__":
    ...
    sequence_tumor = tp53_sequence_normal[:local_position_1+1] + reverse_complement(akain1_sequence_normal[:local_position_2+1])

    # Step 4. Create FASTQ files
    create_fastq_file(
        sequences=[sequence_tumor,akain1_sequence_normal,tp53_sequence_normal],
        output_fastq_file='../../../test/data/fastq/dna-007-tumor_long-read.fastq',
        num_reads=[3,3,3]
    )
    create_fastq_file(
        sequences=[akain1_sequence_normal,tp53_sequence_normal],
        output_fastq_file='../../../test/data/fastq/dna-007-normal_long-read.fastq',
        num_reads=[3,3]
    )
    ...
    pd.DataFrame(data).to_csv('../../../test/data/tsv/ground_truth/dna-007-tumor_ground_truth.tsv', sep='\t', index=False)
    pd.DataFrame(data).to_csv('/Users/leework/Documents/Research/projects/project_exacto/exacto/exacto/exacto-caller/src/tests/data/tsv/ground_truth/dna-007-tumor_ground_truth.tsv', sep='\t', index=False)
