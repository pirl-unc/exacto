...
pub fn identify_dna_variants_in_read(
    record: &bam::Record,
    chromosome: &str
) -> Vec<VariantRecord> {
    ...
    // Step 2. Call breakpoints (BND) using SA tag and CIGAR string
    ...
    if sa_tag.is_empty() == false {
        let mut position_1: u32 = match record.alignment_start().unwrap() {
            Ok(s) => { s.get() as u32 },
            Err(e) => { panic!("Failed to convert: {}", e); },
        };

        // Determine softclip direction
        let mut cigar_idx: u32 = 0;
        ...
        for supplementary_alignment in supplementary_alignments.iter() {
            ...
            let position_2: u32 = parts[1].parse().unwrap();
            let strand: String = parts[2].to_string();
            let cigar: String = parts[3].to_string();
            let mapq: f32 = parts[4].parse().unwrap();
            let nm: u32 = parts[5].parse().unwrap();

            // Record a translocation (BND)
            let variant_record = VariantRecord::new(
                format!("{}:{}:{}", read_id, BREAKPOINT.to_string(), curr_pos).to_string(),
                read_id.to_string(),
                chromosome_1.to_string(),
                position_1,
                chromosome_1.to_string(),
        ...
        let variant_records_list: Vec<Vec<VariantRecord>> = thread_pool.install(|| {
            records.par_iter().map(|record| {
                ...
                if let Some(&(ref chromosome_name, _)) = chromosomes_map.get(&chromosome_idx) {
                    variant_records_ = identify_dna_variants_in_read(
...
pub fn identify_dna_variants(
    ...
    for chromosome in chromosomes.iter() {
        ...
        let mut variant_records: Vec<VariantRecord> = match identify_dna_variant_records(
