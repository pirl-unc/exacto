...
fn test_variant_integration_1() {
    let tsv_path_1 = Path::new("src/tests/data/tsv/dna-001-tumor_minimap2_mdtagged_sorted_exacto_dna_variant_records.tsv");
    let tsv_full_path_1 = fs::canonicalize(tsv_path_1).unwrap();
    let tsv_file_1: &str = tsv_full_path_1.to_str().unwrap();
    let tsv_path_2 = Path::new("src/tests/data/tsv/rna-100-tumor_minimap2_mdtagged_sorted_exacto_rna_variant_records.tsv");
    ...
    let rna_variant_records: Vec<RNAVariantRecord> = load_rna_variant_records(tsv_file_2);
    ...
    let integrated_variants: Vec<IntegratedVariant> = integrate_dna_rna_variants(
        &dna_variant_records,
        &rna_variant_records,
        &gene_annotator,
        2,
    ...
    assert_eq!(integrated_variants.len(), 6);

    let mut rna_variant_call_ids: HashSet<u32> = HashSet::new();
    for integrated_variant in integrated_variants.iter() {
        rna_variant_call_ids.insert(integrated_variant.rna_variant_id);
    }
