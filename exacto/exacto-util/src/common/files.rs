...
pub fn read_fasta_file(fasta_file: &str) -> Vec<(Box<str>,Box<str>)> {
    ...
    let file = File::open(fasta_file).unwrap();
    let mut buf_reader = BufReader::new(file);
    let mut magic = [0; 2];
    buf_reader.read_exact(&mut magic).unwrap();              // Read the first few bytes to check for the gzip magic number (0x1F, 0x8B)
    buf_reader = BufReader::new(buf_reader.into_inner());    // Reset the reader position after reading the magic number
    if magic == [0x1F, 0x8B] {
