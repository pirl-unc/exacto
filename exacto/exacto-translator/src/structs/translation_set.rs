...
impl TranslationSet {
    ...
    pub fn to_dataframe(&self, strategy: TranslationStrategy) -> DataFrame {
        ...
        let mut peptide_ids: Vec<usize> = Vec::new();
        let mut peptide_sequences: Vec<String> = Vec::new();
        let mut rna_ids: Vec<String> = Vec::new();
        let mut rna_sequences: Vec<String> = Vec::new();
        let mut orf_start_positions: Vec<usize> = Vec::new();
        let mut orf_end_positions: Vec<usize> = Vec::new();
        ...
        for translation in self.get_peptides_by_strategy(strategy).iter() {
            for peptide in translation.peptides.iter() {
                ...
                orf_start_positions.push(peptide.orf_start);
                orf_end_positions.push(peptide.orf_end);
                ...
                peptide_ids.push(curr_peptide_id);
            }
        }

        // Convert Vec<usize> to UInt32Chunked and then to Series
        let peptide_ids_ca: UInt64Chunked = ChunkedArray::from_vec("peptide_id".into(), peptide_ids.iter().map(|&x| x as u64).collect());
        let orf_start_positions_ca: UInt32Chunked = ChunkedArray::from_vec("orf_start".into(), orf_start_positions.iter().map(|&x| x as u32).collect());
        let orf_end_positions_ca: UInt32Chunked = ChunkedArray::from_vec("orf_end".into(), orf_end_positions.iter().map(|&x| x as u32).collect());
