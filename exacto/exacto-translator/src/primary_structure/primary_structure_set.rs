...
impl PrimaryStructureSet {
    ...
    pub fn to_dataframe(&self) -> DataFrame {
        ...
        merged_map.insert("amino_acid_index".into(), Vec::new());
        ...
        for primary_structure in self.primary_structures.iter() {
            ...
            // Compute amino_acid_index: increments each time codon_index is 0, -1 for non-base rows
            let codon_indices = primary_structure_records.get("codon_index").unwrap();
            let mut amino_acid_index: i64 = -1;
            for ci in codon_indices.iter() {
                let ci_val: i32 = match ci {
                    AnyValue::Int32(v) => *v,
                    _ => -1,
                };
                if ci_val == 0 {
                ...
                let aai = if ci_val < 0 { -1i64 } else { amino_acid_index };
                merged_map.entry("amino_acid_index".into()).or_default().push(AnyValue::Int64(aai));
            }
        ...
        DataFrame::new(vec![
            ...
            Column::from(Series::new("amino_acid_index".into(), merged_map.get("amino_acid_index").unwrap())),
            ...
            Column::from(Series::new("codon_rna_variant_call_ids".into(), merged_map.get("codon_rna_variant_call_ids").unwrap())),
            Column::from(Series::new("codon_dna_variant_call_ids".into(), merged_map.get("codon_dna_variant_call_ids").unwrap())),
            Column::from(Series::new("frameshift_rna_variant_call_ids".into(), merged_map.get("frameshift_rna_variant_call_ids").unwrap())),
            Column::from(Series::new("frameshift_dna_variant_call_ids".into(), merged_map.get("frameshift_dna_variant_call_ids").unwrap())),
            Column::from(Series::new("amino_acid_change".into(), merged_map.get("amino_acid_change").unwrap()))
        ]).unwrap()
    }
            ...
            for record in primary_structure.records.iter() {
                if *record.get_record_type() == PrimaryStructureRecordType::Base {
                    if record.get_codon_index().unwrap() == 0 {
                        if let Some(amino_acid) = record.get_amino_acid().as_ref() {
                            sequence.push_str(amino_acid);
                        }
                    }
                }
            }
