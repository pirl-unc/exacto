...
impl Gencode {
    ...
    pub fn new(
        ...
        fn extract_all_attrs<'a>(attrs: &'a str, key: &str) -> Vec<Box<str>> {
            let key_prefix = format!("{key} ");
            let mut values: Vec<Box<str>> = Vec::new();
            for field in attrs.split(';') {
                let field = field.trim();
                if field.starts_with(&key_prefix) {
                    let value = field[key_prefix.len()..].trim().trim_matches('"');
        ...
        for line in reader.lines().flatten() {
            ...
            let level: u8 = extract_attr(attrs, "level").and_then(|s| s.parse::<u8>().ok()).unwrap_or(0);
            let exon_number: u16 = extract_attr(attrs, "exon_number").and_then(|s| s.parse::<u16>().ok()).unwrap_or(0);
...
impl GeneAnnotator for Gencode {
    ...
    fn rank_transcripts<'a>(&self, mut transcripts: Vec<&'a Transcript>) -> Vec<&'a Transcript> {
        transcripts.sort_by(|a, b| {
            // Priority 1: protein_coding first
            let type_a = if &*a.transcript_type == "protein_coding" { 0u8 } else { 1 };
            let type_b = if &*b.transcript_type == "protein_coding" { 0u8 } else { 1 };
            type_a.cmp(&type_b)
                // Priority 2: transcript_support_level (1 > 2 > 3 > 4 > 5 > NA)
                .then_with(|| {
                    let tsl_a: u8 = a.transcript_support_level.parse().unwrap_or(6);
                    let tsl_b: u8 = b.transcript_support_level.parse().unwrap_or(6);
                    tsl_a.cmp(&tsl_b)
                })
                // Priority 3: tag (MANE_Select > Ensembl_canonical > basic > none)
                .then_with(|| {
                    let tag_rank = |t: &Transcript| -> u8 {
                        if t.tags.iter().any(|tag| &**tag == "MANE_Select") { 0 }
                        else if t.tags.iter().any(|tag| &**tag == "Ensembl_canonical") { 1 }
                        else if t.tags.iter().any(|tag| &**tag == "basic") { 2 }
                        else { 3 }
                    };
                    tag_rank(a).cmp(&tag_rank(b))
                })
                // Priority 4: APPRIS (principal > alternative > none)
                .then_with(|| {
                    let appris_rank = |t: &Transcript| -> u8 {
                        for tag in t.tags.iter() {
                            if tag.starts_with("appris_principal") { return tag.as_ref().rsplit('_').next().and_then(|n| n.parse::<u8>().ok()).unwrap_or(5) }
                            if tag.starts_with("appris_alternative") { return 10 + tag.as_ref().rsplit('_').next().and_then(|n| n.parse::<u8>().ok()).unwrap_or(2) }
                        }
                        20
                    };
                    appris_rank(a).cmp(&appris_rank(b))
                })
