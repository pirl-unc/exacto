    ...
    for index, row in df_small_variants.iterrows():
        ...
        if len(df_gencode_genes_matched) > 0:
             ...
             if len(df_gencode_exons_matched) > 0:
                 ...
                 curr_pos_exon_number = ','.join(df_gencode_exons_matched['exon_number'].values.tolist())
             else:
             ...
             curr_pos_gene_start = ','.join(df_gencode_genes_matched['gene_start'].values.tolist())
             curr_pos_gene_end = ','.join(df_gencode_genes_matched['gene_end'].values.tolist())
