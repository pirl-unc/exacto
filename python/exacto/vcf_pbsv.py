...
def parse_pbsv_callset(
    ...
    for row in df_vcf.to_dict('records'):
        ...
        variant_call.tool_attributes[VariantCallingMethods.PBSV + '_id'] = safely_retrieve_value(
            dict=row,
            key='ID',
        ...
        variant_call.variant_calling_method = VariantCallingMethods.PBSV
        ...
        for curr_info in info:
            if '=' in curr_info:
                ...
                variant_call.tool_attributes[VariantCallingMethods.PBSV + '_' + curr_info_elements[0].lower()] = safely_convert_value(
                    value=curr_info_elements[1],
                    default_value=DEFAULT_ATTRIBUTE_VALUE,
                    type=VariantCallingMethods.AttributeTypes.PBSV[VariantCallingMethods.PBSV + '_' + curr_info_elements[0].lower()]
                )
            else:
                if curr_info == 'PRECISE':
                    variant_call.tool_attributes[VariantCallingMethods.PBSV + '_precise'] = True
                if curr_info == 'IMPRECISE':
                    variant_call.tool_attributes[VariantCallingMethods.PBSV + '_precise'] = False

        # Extract FORMAT (sample)
        format = str(row['FORMAT']).split(':')
        tumor_sample = str(row[tumor_sample_id]).split(':')
        for curr_format in format:
            variant_call.tool_attributes[VariantCallingMethods.PBSV + '_' + curr_format.lower()] = safely_retrieve_value(
                dict=tumor_sample,
                key=format.index(curr_format),
                default_value=DEFAULT_ATTRIBUTE_VALUE,
                type=VariantCallingMethods.AttributeTypes.PBSV[VariantCallingMethods.PBSV + '_' + curr_format.lower()]
            )

        # Update variant_type
        if VariantCallingMethods.PBSV + '_svtype' in variant_call.tool_attributes.keys():
            variant_call.variant_type = variant_call.tool_attributes[VariantCallingMethods.PBSV + '_svtype']
        ...
        if variant_call.variant_type == VariantTypes.BREAKPOINT or variant_call.variant_type == VariantTypes.TRANSLOCATION:
            if variant_call.tool_attributes[VariantCallingMethods.PBSV + '_id'] in included_mate_ids:
                continue
            included_mate_ids.add(variant_call.tool_attributes[VariantCallingMethods.PBSV + '_id'])

        # Update precise
        if VariantCallingMethods.PBSV + '_precise' in variant_call.tool_attributes.keys():
            variant_call.precise = variant_call.tool_attributes[VariantCallingMethods.PBSV + '_precise']

        # Update pos_2
        if VariantCallingMethods.PBSV + '_end' in variant_call.tool_attributes.keys():
            variant_call.pos_2 = variant_call.tool_attributes[VariantCallingMethods.PBSV + '_end']

        # Update total_tumor_reads
        if VariantCallingMethods.PBSV + '_dp' in variant_call.tool_attributes.keys():
            variant_call.total_tumor_reads = variant_call.tool_attributes[VariantCallingMethods.PBSV + '_dp']

        # Update ref_tumor_reads and alt_tumor_reads
        if VariantCallingMethods.PBSV + '_ad' in variant_call.tool_attributes.keys():
            variant_call.ref_tumor_reads = safely_convert_value(
                value=variant_call.tool_attributes[VariantCallingMethods.PBSV + '_ad'].split(',')[0],
                default_value=DEFAULT_ATTRIBUTE_VALUE,
                type=int
            )
        ...
        # Update variant_size
        if VariantCallingMethods.PBSV + '_svlen' in variant_call.tool_attributes.keys():
            variant_call.variant_size = abs(variant_call.tool_attributes[VariantCallingMethods.PBSV + '_svlen'])

        # Update chr_2 for 'BND'
        if variant_call.variant_type == VariantTypes.BREAKPOINT or variant_call.variant_type == VariantTypes.TRANSLOCATION:
            curr_id = variant_call.tool_attributes[VariantCallingMethods.PBSV + '_id'].split("-")[1]
            variant_call.chr_2 = str(curr_id.split(":")[0])

        # Update pos_2 for 'BND'
        if variant_call.variant_type == VariantTypes.BREAKPOINT or variant_call.variant_type == VariantTypes.TRANSLOCATION:
            curr_id = variant_call.tool_attributes[VariantCallingMethods.PBSV + '_id'].split("-")[1]
            variant_call.pos_2 = int(curr_id.split(":")[1])
