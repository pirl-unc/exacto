...
and run Exacto 'annotate-variant-calls' command.
"""
...
def add_cli_annotate_variant_calls_arg_parser(sub_parsers) -> argparse._SubParsersAction:
    """
    Adds 'annotate-variant-calls' parser.
    ...
    parser_required.add_argument(
        ...
        type=str,
        required=True,
        help="Input TSV file (generated by Exacto)."
    )
    parser_required.add_argument(
        "--annotation-source",
    ...
    parser_optional.add_argument(
        "--gencode-genome-fasta-file",
        dest="gencode_genome_fasta_file",
        type=str,
        required=False,
        help="Genome FASTA file (e.g. 'hg38.fa'; make sure the FASTA file is unzipped). "
             "This parameter must be supplied if "
             "--annotation_source is '%s'."
             % AnnotationSources.GENCODE
    )
...
def run_cli_annotate_variant_calls_from_parsed_args(args) -> None:
    """
    Run Exacto 'annotate-variant-calls' command using parameters from parsed arguments.
    ...
    if args.annotation_source == AnnotationSources.ENSEMBL:
        ...
        annotation_db = Gencode(
            ...
            genome_fasta_file=args.gencode_genome_fasta_file,
            version=args.gencode_version,
            genome=args.gencode_genome
        )
    else:
        raise Exception('Unknown annotation source: %s' % args.annotation_source)

    # Step 3. Annotate variants
    variants_list = run_exacto_annotate_variants_list(
