...
and run Exacto 'call-variants' command.
"""
...
from .main import run_exacto_call_variants


def add_cli_call_variants_arg_parser(
        sub_parsers
    ) -> argparse._SubParsersAction:
    """
    Adds 'call-variants' parser.
    ...
    parser_required.add_argument(
        ...
        help="Nucleic acid type. Allowed options: %s."
             % (', '.join([NucleicAcidTypes.DNA, NucleicAcidTypes.RNA]))
    )
    parser_required.add_argument(
        "--bam-file",
        dest="bam_file",
    ...
    parser.set_defaults(which='call-variants')
    return sub_parsers


def run_cli_call_variants_from_parsed_args(
        args
    ) -> None:
    """
    Run Exacto 'call-variants' command using parameters from parsed arguments.
    ...
    if args.nucleic_acid_type == NucleicAcidTypes.RNA:
        ...
        df_variants = run_exacto_call_variants(
            bam=bam,
