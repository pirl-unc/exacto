...
and run Exacto 'call-germline-dna-vars' command.
"""
...
def add_cli_call_germline_dna_vars_arg_parser(sub_parsers) -> argparse._SubParsersAction:
    """
    Add 'call-germline-dna-vars' parser.
    ...
    Returns:
        sub_parsers     :   argparse.ArgumentParser subparsers
    """
    parser = sub_parsers.add_parser('call-germline-dna-vars', help='Call germline DNA variants in a long-read DNA BAM file.')
    ...
    parser_optional.add_argument(
        ...
        dest="num_threads",
        type=int,
        default=CALL_GERMLINE_DNA_VARS_NUM_THREADS,
        required=False,
        help="Number of threads (default: %i)."
             % CALL_GERMLINE_DNA_VARS_NUM_THREADS
    )
    parser_optional.add_argument(
        '--regions',
        dest='regions',
    ...
    parser_optional.add_argument(
        ...
        dest="min_reads",
        type=int,
        default=CALL_GERMLINE_DNA_VARS_MIN_READS,
        required=False,
        help="Minimum number of supporting reads (default: %i)."
             % CALL_GERMLINE_DNA_VARS_MIN_READS
    )
    parser_optional.add_argument(
        "--min-mapping-quality",
        dest="min_mapping_quality",
        type=int,
        default=CALL_GERMLINE_DNA_VARS_MIN_MAPPING_QUALITY,
        required=False,
        help="Minimum mapping quality (default: %i)."
             % CALL_GERMLINE_DNA_VARS_MIN_MAPPING_QUALITY
    )
    parser_optional.add_argument(
        "--min-base-quality",
        dest="min_base_quality",
        type=int,
        default=CALL_GERMLINE_DNA_VARS_MIN_BASE_QUALITY,
        required=False,
        help="Minimum base quality (default: %i)."
             % CALL_GERMLINE_DNA_VARS_MIN_BASE_QUALITY
    )
    parser_optional.add_argument(
        "--min-total-depth",
        dest="min_total_depth",
        type=int,
        default=CALL_GERMLINE_DNA_VARS_MIN_TOTAL_DEPTH,
        required=False,
        help="Minimum total depth (default: %i)."
             % CALL_GERMLINE_DNA_VARS_MIN_TOTAL_DEPTH
    )
    parser_optional.add_argument(
        "--min-alt-allele-fraction",
        dest="min_alt_allele_fraction",
        type=float,
        default=CALL_GERMLINE_DNA_VARS_MIN_ALT_ALLELE_FRACTION,
        required=False,
        help="Minimum alternate allele fraction (default: %f)."
             % CALL_GERMLINE_DNA_VARS_MIN_ALT_ALLELE_FRACTION
    )
    parser_optional.add_argument(
        "--min-size-proportion",
        dest="min_size_proportion",
        type=float,
        default=CALL_GERMLINE_DNA_VARS_MIN_SIZE_PROPORTION,
        required=False,
        help="Minimum size proportion between two variants (default: %f). "
             "Size proportion = smaller variant size / longer variant size."
             % CALL_GERMLINE_DNA_VARS_MIN_SIZE_PROPORTION
    )
    parser_optional.add_argument(
        "--max-ins-norm-edit-distance",
        dest="max_ins_norm_edit_distance",
        type=float,
        default=CALL_GERMLINE_DNA_VARS_MAX_INS_NORM_EDIT_DISTANCE,
        required=False,
        help="Maximum insertion normalized edit (Levenshtein) distance (default: %f). "
             ...
             % CALL_GERMLINE_DNA_VARS_MAX_INS_NORM_EDIT_DISTANCE
    )
    parser_optional.add_argument(
        "--max-intrachromosomal-distance",
        dest="max_intrachromosomal_distance",
        type=int,
        default=CALL_GERMLINE_DNA_VARS_MAX_INTRACHROMOSOMAL_DISTANCE,
        required=False,
        help="Maximum distance for clustering intrachromomsomal variants (default: %i)."
             % CALL_GERMLINE_DNA_VARS_MAX_INTRACHROMOSOMAL_DISTANCE
    )
    parser_optional.add_argument(
        "--max-intrachromosomal-distance-tau",
        dest="max_intrachromosomal_distance_tau",
        type=int,
        default=CALL_GERMLINE_DNA_VARS_MAX_INTRACHROMOSOMAL_DISTANCE_TAU,
        required=False,
        help="Maximum distance tau for clustering intrachromomsomal variants (default: %i)."
             % CALL_GERMLINE_DNA_VARS_MAX_INTRACHROMOSOMAL_DISTANCE_TAU
    )
    parser_optional.add_argument(
        "--max-interchromosomal-distance",
        dest="max_interchromosomal_distance",
        type=int,
        default=CALL_GERMLINE_DNA_VARS_MAX_INTERCHROMOSOMAL_DISTANCE,
        required=False,
        help="Maximum distance for clustering intrachromomsomal variants (default: %i)."
             % CALL_GERMLINE_DNA_VARS_MAX_INTERCHROMOSOMAL_DISTANCE
    )
    parser_optional.add_argument(
        "--max-slippage-repeat-length",
        dest="max_slippage_repeat_length",
        type=int,
        default=CALL_GERMLINE_DNA_VARS_MAX_SLIPPAGE_REPEAT_LENGTH,
        required=False,
        help="Maximum slippage repeat length (default: %i)."
             % CALL_GERMLINE_DNA_VARS_MAX_SLIPPAGE_REPEAT_LENGTH
    )
    parser_optional.add_argument(
        "--chunk-size",
        dest="chunk_size",
        type=int,
        default=CALL_GERMLINE_DNA_VARS_CHUNK_SIZE,
        required=False,
        help="Chunk size for variant calling (default: %i)." % CALL_GERMLINE_DNA_VARS_CHUNK_SIZE
    )
    parser_optional.add_argument(
        "--max-records",
        dest="max_records",
        type=int,
        default=CALL_GERMLINE_DNA_VARS_MAX_RECORDS,
        required=False,
        help="Maximum number of records. Read names having more than this value will be excluded (default: %i)."
             % CALL_GERMLINE_DNA_VARS_MAX_RECORDS
    )
    parser_optional.add_argument(
        "--expected-variant-allele-fraction",
        dest="expected_variant_allele_fraction",
        type=float,
        default=CALL_GERMLINE_DNA_VARS_EXPECTED_VARIANT_ALLELE_FRACTION,
        required=False,
        help="Expected variant allele fraction (default: %f)."
             % CALL_GERMLINE_DNA_VARS_EXPECTED_VARIANT_ALLELE_FRACTION
    )
    parser_optional.add_argument(
        "--expected-mutation-rate",
        dest="expected_mutation_rate",
        type=float,
        default=CALL_GERMLINE_DNA_VARS_EXPECTED_MUTATION_RATE,
        required=False,
        help="Expected mutation rate (default: %f)."
             % CALL_GERMLINE_DNA_VARS_EXPECTED_MUTATION_RATE
    )
    parser_optional.add_argument(
        ...
        help="Expected sequencing error rate (default: %f with no preset, %f with --preset pb, %f with --preset ont)."
             % (CALL_GERMLINE_DNA_VARS_EXPECTED_SEQUENCING_ERROR,
    ...
    parser_optional.add_argument(
        ...
        help="Expected slippage probability (default: %f with no preset, %f with --preset pb, %f with --preset ont). "
             ...
             % (CALL_GERMLINE_DNA_VARS_EXPECTED_SLIPPAGE_PROBABILITY,
    ...
    parser_optional.add_argument(
        ...
        type=float,
        default=CALL_GERMLINE_DNA_VARS_MAX_F1_FRACTION,
        required=False,
        help="Maximum F1 value fraction (default: %f)."
             % CALL_GERMLINE_DNA_VARS_MAX_F1_FRACTION
    )
    parser_optional.add_argument(
        "--max-fpr",
        dest="max_fpr",
        type=float,
        default=CALL_GERMLINE_DNA_VARS_MAX_FPR,
        required=False,
        help="Maximum false positive rate (default: %f)."
             % CALL_GERMLINE_DNA_VARS_MAX_FPR
    )
    parser_optional.add_argument(
        "--temp-dir",
        dest="temp_dir",
    ...
    parser.set_defaults(which='call-germline-dna-vars')
    return sub_parsers


def run_cli_call_germline_dna_vars_from_parsed_args(args):
    """
    Run Exacto 'call-germline-dna-vars' command using parameters from parsed arguments.
    ...
    preset_values: dict[str, float] = CALL_DNA_VARS_PRESETS.get(args.preset, {})
    expected_sequencing_error = (
        args.expected_sequencing_error
        if args.expected_sequencing_error is not None
        else preset_values.get("expected_sequencing_error", CALL_GERMLINE_DNA_VARS_EXPECTED_SEQUENCING_ERROR)
    )
    expected_slippage_probability = (
        args.expected_slippage_probability
        if args.expected_slippage_probability is not None
        else preset_values.get("expected_slippage_probability", CALL_GERMLINE_DNA_VARS_EXPECTED_SLIPPAGE_PROBABILITY)
    )
