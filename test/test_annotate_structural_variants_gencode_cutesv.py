...
def test_annotate_dna_structural_variants_gencode_cutesv():
    ...
    assert 'lncRNA' in df_sv_cutesv_annotated['gencode_pos_1_gene_type'].values.tolist(), \
        "The value 'lncRNA' is expected to appear in the column 'gencode_pos_1_gene_type'"
    assert 'ENSG00000230021' in df_sv_cutesv_annotated['gencode_pos_1_gene_name'].values.tolist(), \
